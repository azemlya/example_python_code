...
class HiveCliHookSE(BaseHook):
    ...
    conn_name_attr = "hive_cli_se_conn_id"
    default_conn_name = "hive_cli_se_default"
    conn_type = "hive_cli_se"
    hook_name = "Hive Client Wrapper SE"

    def __init__(
        self,
        hive_cli_se_conn_id: str = default_conn_name,
        ...
        super().__init__()
        conn = self.get_connection(hive_cli_se_conn_id)
...
class HiveMetastoreHookSE(BaseHook):
    """
    ...
    conn_name_attr = "metastore_se_conn_id"
    default_conn_name = "metastore_se_default"
    conn_type = "hive_metastore_se"
    hook_name = "Hive Metastore Thrift SE"
    ...
    def check_for_partition(self, schema: str, table: str, partition: str) -> bool:
        ...
        with self.metastore as client:
            partitions = client.get_partitions_by_filter(
                schema, table, partition, HiveMetastoreHookSE.MAX_PART_COUNT
            )
            ...
            if table.partitionKeys:
                if partition_filter:
                    parts = client.get_partitions_by_filter(
                        ...
                        max_parts=HiveMetastoreHookSE.MAX_PART_COUNT,
                    )
                else:
                    parts = client.get_partitions(
                        db_name=schema, tbl_name=table_name, max_parts=HiveMetastoreHookSE.MAX_PART_COUNT
                    )
    ...
    def max_partition(
        ...
        with self.metastore as client:
            ...
            part_names = client.get_partition_names(
                schema, table_name, max_parts=HiveMetastoreHookSE.MAX_PART_COUNT
            )
            part_specs = [client.partition_name_to_spec(part_name) for part_name in part_names]

        return HiveMetastoreHookSE._get_max_partition_from_part_specs(part_specs, field, filter_map)
...
class HiveServer2HookSE(DbApiHook):
    """
    ...
    * the default for run_set_variable_statements is true, if you
    ...
    conn_name_attr = "hiveserver2_se_conn_id"
    default_conn_name = "hiveserver2_se_default"
    conn_type = "hiveserver2_se"
    hook_name = "Hive Server 2 Thrift SE"
