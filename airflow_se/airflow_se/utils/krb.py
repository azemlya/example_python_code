...
def run_kvno(
    ...
    """
    """
    assert isinstance(ticket, str) and not ticket.isspace(), 'Parameter "ticket" must be set'
    assert isinstance(service, str) and not service.isspace(), 'Parameter "service" must be set'
    return run_command(cmd=f'kvno -c {ticket} {service}')
