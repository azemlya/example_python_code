...
class NamedHivePartitionSensorSE(BaseSensorOperator):
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.hook = hook
        if self.hook and metastore_se_conn_id != "metastore_default":
            self.log.warning(
                "A hook was passed but a non default metastore_se_conn_id=%s was used", metastore_se_conn_id
            )
    ...
    def poke_partition(self, partition: str) -> Any:
        ...
        if not self.hook:
            ...
            self.hook = HiveMetastoreHookSE(metastore_se_conn_id=self.metastore_se_conn_id)
